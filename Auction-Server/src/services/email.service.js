 ...
 * Triggered after successful Cashfree payment for tournament hosting plan
 */
export const sendOrganizerPackInvoiceEmail = async ({ organizer, plan, payment, tournament }) => {
  if (!organizer.email) {
    console.log(`ℹ️ Organizer ${organizer.name} has no email. Skipping pack invoice.`);
    return;
  }
  ...
  try {
    ...
    const transactionId = payment.transactionId || payment.orderId || 'N/A';
    const paymentMethod = payment.method || 'Cashfree';
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Invoice ${invoiceNo} - Pack Purchase</title>
      <style>
        @media only screen and (max-width: 480px) {
          .outer-pad { padding: 8px !important; }
          .inner-pad { padding: 24px 18px !important; }
          ...
          <td align="center" class="outer-pad" style="padding:32px 16px;">
            ...
            <table cellpadding="0" cellspacing="0" border="0" style="max-width:560px; width:100%; background:#ffffff; border-radius:8px; overflow:hidden;">
              ...
              <tr>
                <td style="padding:28px 36px 16px 36px; text-align:center;">
                  <h1 style="margin:0 0 4px 0; font-size:28px; font-weight:900; color:#1e1e2f; letter-spacing:2px;">INVOICE</h1>
                  <p style="margin:0; font-size:13px; color:#888888; font-weight:500;">Auction Hosting Plan Purchase</p>
                </td>
              </tr>
              ...
              <tr>
                <td class="inner-pad" style="padding:0 36px 8px 36px;">
                  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
                    ...
                    <!-- Row 1: Auction Hosting Plan -->
                    <tr>
                      <td align="left" style="padding:14px 12px; font-size:13px; color:#333333; border-bottom:1px solid #f0f0f0;">
                        ${planName} (Up to ${maxTeams} Teams)
                      </td>
                    ...
                    <tr>
                      ...
                      <td align="right" style="width:50%;">
                        <table cellpadding="0" cellspacing="0" border="0" style="width:100%;">
                          <tr>
                            <td align="right" style="padding:8px 16px 8px 0; font-size:16px; font-weight:800; color:#1e1e2f;">TOTAL</td>
                            <td align="right" style="padding:8px 0; font-size:22px; font-weight:900; color:#f5a623;">₹${planPrice}</td>
                          </tr>
                        </table>
                      </td>
                    </tr>
              ...
              <tr>
                <td class="inner-pad" style="padding:18px 36px 20px 36px;">
                  ...
                  <table cellpadding="0" cellspacing="0" border="0" style="width:100%;">
                    ...
                    <tr>
                      <td style="padding:4px 0; font-size:12px; font-weight:600; color:#555555;">Order ID:</td>
                      <td style="padding:4px 0; font-size:13px; color:#333333; word-break:break-all;">${orderId}</td>
                    </tr>
                    <tr>
                      <td style="padding:4px 0; font-size:12px; font-weight:600; color:#555555;">Transaction ID:</td>
                      <td style="padding:4px 0; font-size:13px; color:#333333; word-break:break-all;">${transactionId}</td>
                    </tr>
                    <tr>
                      <td style="padding:4px 0; font-size:12px; font-weight:600; color:#555555;">Status:</td>
                      <td style="padding:4px 0; font-size:13px; font-weight:700; color:#16a34a;">✓ Paid & Verified</td>
              ...
              <!-- ── Plan Features / What You Get ── -->
              <tr>
                <td class="inner-pad" style="padding:18px 36px 20px 36px;">
                  <p style="margin:0 0 10px 0; font-size:12px; font-weight:700; color:#1e1e2f; text-transform:uppercase; letter-spacing:0.5px;">📋 Your Plan Includes</p>
                  <table cellpadding="0" cellspacing="0" border="0" style="width:100%;">
                    <tr>
                      <td style="padding:5px 0; font-size:13px; color:#333333;">✅ Auction hosting for up to <strong>${maxTeams} teams</strong></td>
                    </tr>
                    <tr>
                      <td style="padding:5px 0; font-size:13px; color:#333333;">✅ Live auction room with real-time bidding</td>
                    </tr>
                    <tr>
                      <td style="padding:5px 0; font-size:13px; color:#333333;">✅ Player registration & management</td>
                    </tr>
                    <tr>
                      <td style="padding:5px 0; font-size:13px; color:#333333;">✅ Team budget tracking & analytics</td>
                    </tr>
                    <tr>
                      <td style="padding:5px 0; font-size:13px; color:#333333;">✅ Shareable tournament dashboard</td>
                    </tr>
                    ${planDescription ? `<tr><td style="padding:8px 0 0 0; font-size:12px; color:#999999; font-style:italic;">${planDescription}</td></tr>` : ''}
                  </table>
                </td>
              </tr>
              ...
              <tr>
                <td class="inner-pad" style="padding:18px 36px 24px 36px;">
                  <p style="margin:0 0 8px 0; font-size:12px; font-weight:700; color:#1e1e2f; text-transform:uppercase; letter-spacing:0.5px;">📌 Terms & Conditions</p>
                  <p style="margin:0; font-size:11px; color:#999999; line-height:1.6;">
                    This is a one-time payment for a single auction event. The plan is activated immediately upon successful payment.
                    No refunds once the auction has been started. For support or queries, please visit cricauctionhub.vercel.app or contact our team.
                  </p>
                </td>
              </tr>
    ...
    // 1. Primary: Brevo REST API
    if (brevoApiKey) {
      try {
        console.log(`✉️ Attempting pack invoice dispatch via Brevo REST API to ${organizer.email}...`);
        const response = await fetch("https://api.brevo.com/v3/smtp/email", {
          method: "POST",
          headers: {
            "accept": "application/json",
          ...
          body: JSON.stringify({
            sender: { name: "CricAuction Hub", email: fromEmail },
            to: [{ email: organizer.email, name: organizerName }],
        ...
        const data = await response.json();
        if (response.ok) {
          console.log(`✅ Pack invoice email sent via Brevo to ${organizer.email}. Message ID: ${data.messageId}`);
          return data;
        } else {
          console.warn(`⚠️ Brevo API returned error:`, data);
        }
    ...
    const info = await transporter.sendMail(mailOptions);
    console.log(`✅ Pack invoice email sent via SMTP to ${organizer.email}. Message ID: ${info.messageId}`);
    return info;
...
export const sendOrganizerUpgradeInvoiceEmail = async ({ organizer, oldPlan, newPlan, netPaid, payment, tournament }) => {
  if (!organizer.email) {
    console.log(`ℹ️ Organizer ${organizer.name} has no email. Skipping upgrade invoice.`);
    return;
  }
  ...
  try {
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Upgrade Invoice ${invoiceNo}</title>
      <style>
        @media only screen and (max-width: 480px) {
          .outer-pad { padding: 8px !important; }
          .inner-pad { padding: 24px 18px !important; }
        ...
        <tr>
          <td align="center" class="outer-pad" style="padding:32px 16px;">
            <table cellpadding="0" cellspacing="0" border="0" style="max-width:560px; width:100%; background:#ffffff; border-radius:8px; overflow:hidden;">
              ...
              <tr>
                <td style="padding:28px 36px 16px 36px; text-align:center;">
                  <h1 style="margin:0 0 4px 0; font-size:26px; font-weight:900; color:#1e1e2f; letter-spacing:1px;">⚡ PLAN UPGRADE INVOICE</h1>
                  <p style="margin:0; font-size:13px; color:#888888; font-weight:500;">Difference Amount Payment Receipt</p>
                </td>
              </tr>

              <tr>
                <td style="padding:0 36px;">
                  <div style="background:#f5a623; border-radius:8px; padding:14px 20px; text-align:center;">
              ...
              <tr>
                <td class="inner-pad" style="padding:0 36px 12px 36px;">
                  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
                    <tr>
                      <th align="left" style="background:#f5a623; padding:10px 12px; font-size:11px; font-weight:700; color:#ffffff; border-radius:6px 0 0 0;">Description</th>
                      <th align="right" style="background:#f5a623; padding:10px 12px; font-size:11px; font-weight:700; color:#ffffff; border-radius:0 6px 0 0;">Amount</th>
                    </tr>
                    <tr>
              ...
              <tr>
                <td style="padding:16px 36px 24px 36px; text-align:center; background:#f9f9f9; border-top:1px solid #eeeeee;">
                  <p style="margin:0; font-size:12px; color:#999999;">
                    Thank you! Your tournament limit is now set to <strong>${newPlan.maxTeams} teams</strong>. 🏏<br>
                    <a href="https://cricauctionhub.vercel.app" style="color:#f5a623; text-decoration:none; font-weight:600;">cricauctionhub.vercel.app</a>
                  </p>
                </td>
              </tr>
    ...
    if (brevoApiKey) {
      ...
      } catch (e) { console.error('Brevo upgrade email err:', e); }
    }
 ...
 */
export const sendOrganizerCancellationRefundEmail = async ({ organizer, plan, refundAmount, refundId, payment, tournament }) => {
  if (!organizer.email) {
    console.log(`ℹ️ Organizer ${organizer.name} has no email address. Skipping cancellation email.`);
    return;
  }
  ...
  try {
    ...
    const cancelDate = new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Cancellation & Refund ${invoiceNo}</title>
      <style>
        @media only screen and (max-width: 480px) {
          .outer-pad { padding: 8px !important; }
          .inner-pad { padding: 24px 18px !important; }
        ...
        <tr>
          <td align="center" class="outer-pad" style="padding:32px 16px;">
            <table cellpadding="0" cellspacing="0" border="0" style="max-width:560px; width:100%; background:#ffffff; border-radius:8px; overflow:hidden;">
              ...
              <tr>
                <td style="padding:28px 36px 16px 36px; text-align:center;">
                  <h1 style="margin:0 0 4px 0; font-size:24px; font-weight:900; color:#dc2626; letter-spacing:1px;">❌ CANCELLATION & REFUND</h1>
                  <p style="margin:0; font-size:13px; color:#888888;">Tournament Plan Refund Confirmation</p>
                </td>
              </tr>

              <tr>
                <td style="padding:0 36px;">
                  <div style="background:#dc2626; border-radius:8px; padding:14px 20px; text-align:center;">
                    <span style="font-size:17px; font-weight:800; color:#ffffff;">🏏 CricAuction Hub</span>
                  </div>
                </td>
              </tr>
              ...
              <tr>
                <td class="inner-pad" style="padding:20px 36px;">
                  ...
                  <p style="margin:0 0 14px 0; font-size:13px; color:#555555; line-height:1.5;">
                    Your subscription plan for <strong>${tournamentName}</strong> has been cancelled per your request, and a full refund has been initiated to your original payment method.
                  </p>
                  ...
                  <div style="background:#f9fafb; border:1px solid #e5e7eb; border-radius:8px; padding:12px 16px;">
                    ...
                    <p style="margin:0; font-size:12px; color:#16a34a; font-weight:700;">Status: ✓ Refund Processed / Initiated</p>
                  </div>
                </td>
              </tr>
              ...
              <tr>
                <td style="padding:16px 36px 24px 36px; text-align:center; background:#f9f9f9; border-top:1px solid #eeeeee;">
                  <p style="margin:0 0 4px 0; font-size:12px; color:#999999;">
                    The refund will reflect in your UPI / Bank Account within 5 minutes to 24 hours.
                  </p>
                  <p style="margin:0; font-size:11px; color:#bbbbbb;">
                    Questions? Contact support at <a href="https://cricauctionhub.vercel.app" style="color:#dc2626; text-decoration:none; font-weight:600;">cricauctionhub.vercel.app</a>
                  </p>
                </td>
              </tr>
      ...
      try {
        const response = await fetch("https://api.brevo.com/v3/smtp/email", {
          ...
          body: JSON.stringify({
            sender: { name: "CricAuction Hub", email: fromEmail },
            to: [{ email: organizer.email, name: organizerName }],
            subject: `❌ Subscription Cancelled & Refund Initiated (${invoiceNo})`,
            htmlContent
          })
        });
        if (response.ok) return await response.json();
      } catch (e) { console.error('Brevo refund email err:', e); }
    }
